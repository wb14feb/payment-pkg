...
class DokuCheckoutService implements PaymentServiceContract
{
	...
	public function initiate(PaymentRequest $request): PaymentResponse
	{
		...
		$checkoutUrl = $this->extractCheckoutUrl($responseData, $paymentData);

		return new PaymentResponse(
			success: empty($response['error_messages']),
			transactionId: $paymentData['token_id'] ?? $request->orderId,
	...
	private function buildPayload(PaymentRequest $request, ?string $paymentMethodType = null): array
	{
        ...
        $paymentMethodTypes = [];
        foreach (explode(',', $this->config['services']['doku']['available_methods'] ?? '') as $type) {
            $paymentMethodTypes[] = $this->mapChannelType($type);
        }
		...
		$payload = [
			...
			'payment' => [
				'payment_due_date' => 60,
				'type' => 'SALE',
                'payment_method_types' => !empty($paymentMethodTypes) && !empty($paymentMethodTypes[0]) ? $paymentMethodTypes : null,
			],
			'customer' => [
				'id' => $request->orderId,
				'name' => $firstName,
	...
	private function buildLineItems(PaymentRequest $request): array
	{
		...
		if ($request->getAdminFeeValue() > 0) {
			$lineItems[] = [
				...
				'name' => $this->sanitizeItemField($request->getAdminFeeName()),
				'quantity' => 1,
		...
		if (empty($lineItems)) {
			$lineItems[] = [
				'id' => '1',
				'name' => $this->sanitizeItemField($request->description),
				'quantity' => 1,
				'price' => (int) round($request->amount),
				'sku' => $this->sanitizeItemField(Str::limit($request->orderId, 64, '')),
				'category' => $this->sanitizeItemField('general'),
				'type' => $this->sanitizeItemField('payment'),
			];
		}
	...
	private function mapItem(PaymentItemRequest $item, int $index): array
	{
		return [
			'id' => (string) ($index + 1),
			'name' => $this->sanitizeItemField($item->name),
			'quantity' => $item->quantity,
			'price' => (int) round($item->price),
			'sku' => $this->sanitizeItemField($item->sku),
			'category' => $this->sanitizeItemField($item->category ?: 'general'),
			'type' => $this->sanitizeItemField($item->brand ?: 'product'),
		];
	}

	private function sanitizeItemField(?string $value): string
	{
	...
	private function extractCheckoutUrl(array $responseData, array $paymentData): ?string
	{
